...
def weighted_mse(y_obs, y_pred): #loss function
...
idx_train,idx_test,idx_val = get_train_test_val_idx(predictors.msl.values,surge_obs,[.6,.2,.2],0) #split data into training, validation and testing and get indices

model = build_CNN_model(2,1,92,32,predictors,['msl','w','u10','v10'],'model_0',0,0.0001,weighted_mse,0.01)


#train the model with the train set (hyperparameters are not tuned in this case)
...
### do some evaluation
threshold_pct = 95 #percentile of storm surge data to look at
...
### train a multi-linear regression model (Tadesse et al., 2020)
